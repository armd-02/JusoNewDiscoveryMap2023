...
class ListTable {
	...
	makeList() {
		...
		for (const target of Conf.listTable.targets) {
			...
			targets = targets.concat(mod_target);
		}
	...
	filterCategory(categorys) {		// subset of change list_category
		...
		if (this.list.length > 0) {
			this.flist = categorys !== "-" ? this.#filter(categorys, this.list[0].length - 1) : this.list;	// 最後の列を指定してフィルタリング
		...
		for (const target of targets) {
			switch (target) {
				case "targets":		// ターゲットリストからカテゴリを表示
					pois = Conf.PoiView.targets.map(name => { return [glot.get(`target_${name}`), name] });
					pois.map(poi => {
				...
				default:			// 指定したターゲットからカテゴリを表示
					...
					pois.filter(Boolean).sort().map(poi => {
						if (poi[poi.length - 1].join(",").indexOf(target) > -1) {
							winCont.select_add(`list_category`, poi[1], target + "," + poi[poi.length - 2]);	// target + 元タグ
