						...
						default:
							...
							Conf.list.columns.act_fields.forEach(key => {
								if (key.indexOf("datetime") > -1) {			// フィールド名に日時を含む場合
									...
									let osm_data = poiCont.get_osmid(line.osmid);
									if (osm_data !== undefined) data.push(osm_data.geojson.properties[tag_name]);	// OSM tag名を指定した場合
								} else {
...
class poiMarker {
	...
	static set(target, actonly, flist) {
		...
		const make_marker = function (params) {		// Make Marker(Sometimes multiple markers are returned)
			return new Promise((resolve, reject) => {
				...
				params.html = `<div class="d-flex align-items-center">`;
				...
				let span = `<span class="icon_${css_name} fs-${Conf.effect.text.size}">${name}</span>`;
				if (name !== "" && Conf.effect.text.view) params.html += span;
				resolve([leaflet.addMarker(params)]);
			});
		};
	...
	static select(poiid, detail) {								// Map move to PoiId & Zoom(config)
		return new Promise((resolve, reject) => {
			...
			let zoomlv = Conf.PoiViewZoom["activity"] >= leaflet.getZoom() ? Conf.PoiViewZoom["activity"] : leaflet.getZoom();
			if (poi !== undefined) {	// poi = osmid
				leaflet.flyTo(poi.latlng, zoomlv, { animate: true, duration: 0.5 });
				...
				if (osmid !== undefined) {	// Found Poi
					leaflet.flyTo(osmid.latlng, zoomlv, { animate: true, duration: 0.5 });
	...
	static delete(target, osmid) {					// Marker delete * don't set pdata
		if (osmid == undefined || osmid == "") {	// all osmid
			...
			leaflet.delMaker(poiMarker.#markers[target][idx]);
		};
	}
};
