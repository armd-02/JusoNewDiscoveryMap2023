...
class poiCont {
	...
	static setActdata(json) { poiCont.adata = json };		// set GoogleSpreadSheetから帰ってきたJson
	...
	static getCatnames(tags) {		// get Category Name & subname & tag
	...
	static getTarget(target) {											// 指定したtargetのpoisとactsを返す
	...
	static get_pois(targets) {		// 指定したtargetsのpoisを返す
		let tars = [], lls = [];
	...
	static makeList(targets) {
		let LL = MapLibre.get_LL();
		let pois = poiCont.get_pois(targets), datas = [], listed = {};			// targetsに指定されたpoiのみフィルター
				...
				if (line !== undefined) {
					...
					switch (newmode) {
						...
						default:
							...
							if (poi !== undefined) {
								if (geoCont.checkInner(poi.lnglat, LL) && listed[poi.geojson.properties.id] !== true) {
	...
	setPoi(target, actonly, flist) {
		...
		const checkMarker = function (poi, LL, zoomlv) {
			...
			if (geoCont.checkInner(poi.lnglat, LL)) {
				...
				let tg = actlists.length > 0 ? Conf.google.targetName : target;
				if (viewflag && !poiMarker.#osmpois.includes(poi.geojson.id) && zoomlv >= Conf.PoiViewZoom[tg]) {		// act ok and 未表示の場合
		...
		const saveMarker = function (params) {
			poiMarker.#osmpois.push(params.id);
		...
		if (flist !== undefined) {	// set flist
			...
			if (all.pois.geojson !== undefined) {	// pois表示
				...
				all.pois.geojson.forEach(function (geojson, idx) {
					...
					let params = checkMarker(poi, LL, zoomlv);
					if (Object.keys(params).length > 0) {		// check ok(params内容あり)
						poisOrder[params.actlists.length == 0 ? "unshift" : "push"](params);
					};
				});
				...
				all.acts.forEach((act) => {
					if (!poiMarker.#osmpois.includes(act.osmid)) {
						...
						if (osm !== undefined && (act.category == org_target || org_target == "activity")) {
							...
							if (geoCont.checkInner(poi.lnglat, LL)) saveMarker(params);
						} else {
